...
- [x] `tests/conformance.spec.js` gains **Part D — Agent mode** with, at 1440×900 on the mock:
...
- [x] `tests/recon-values.js` gains an `AGENT` block naming its sources (RECON-04 §7 for the pill; RECON-10 §1/§4 for structure; `EST:` for panel geometry)
- [x] The whole suite stays green on the mock; Part D adds no new snapshot baselines
